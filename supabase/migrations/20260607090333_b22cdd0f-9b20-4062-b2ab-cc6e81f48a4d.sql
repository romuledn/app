...
AS $function$
DECLARE inv RECORD; paid_sum numeric; q_valid date; pct numeric;
BEGIN
  ...
  pct := CASE WHEN inv.total > 0 THEN (paid_sum / inv.total) ELSE 0 END;

  -- Auto-create project once 60% of the invoice has been paid
  IF pct >= 0.6 THEN
    IF inv.quotation_id IS NOT NULL THEN
      SELECT valid_until INTO q_valid FROM public.quotations WHERE id = inv.quotation_id;
      INSERT INTO public.projects(user_id, client_id, quotation_id, title, status, start_date, due_date)
      VALUES (inv.user_id, inv.client_id, inv.quotation_id, inv.title, 'ongoing', CURRENT_DATE, COALESCE(q_valid, CURRENT_DATE + INTERVAL '30 days'))
    ...
    ELSE
      IF NOT EXISTS (
        ...
        INSERT INTO public.projects(user_id, client_id, title, status, start_date, due_date)
        VALUES (inv.user_id, inv.client_id, inv.title, 'ongoing', CURRENT_DATE, CURRENT_DATE + INTERVAL '30 days');
      END IF;
    END IF;
  END IF;

  -- Receipt only on full payment
